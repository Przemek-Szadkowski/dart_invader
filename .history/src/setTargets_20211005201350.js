...
class Target {
    ...
    bindElementsToDrawing(element, category, numbersSet) {
        element.textContent = `${category}${numbersSet[newTarget]}`;
    ...
    drawNumber(element, category = '', special = false) {
        if(special) {
            ...
            this.bindElementsToDrawing(element, category, specialSet);
            // const newTarget = Math.floor(Math.random() * specialSet.length);
            ...
            const newTarget = Math.floor(Math.random() * specialSet.length);
            this.bindElementsToDrawing(element, category, this.bigNumbers);
